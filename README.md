...
## termとして8進数や16進数
s-token.cのtoken関数を修正し、8進数/16進数を入力できるようにした。
8進数入力は```0```で始まり、16進数入力は```0x```で始まるため、その入力を受けた際に、正しく値を受け取れるように変更する必要があった。
...
## -1 や -a 
-から始まるtermをtoken()した時に、それが```-term```であると読み込むことができれば良い。
